...
<div class="applied">
	<div class="row">
		<div class="col-xs-8">
			...
			<fmt:message key="post.traffic.inspector.junior" />
		</div>
		<div class="col-xs-4">
 ...
 <form class="form-horizontal">
	<table style="width:100%">
			...
			<tr>
					<th style="width:25%">
						<label><fmt:message
								key="trafficInspectorJunior.mscitOrGaccc" />:</label>
					</th>
					<td style="width:25%">
					...
					<th style="width:25%">
						<label><fmt:message
								key="trafficInspectorJunior.heightWithinRange" />:</label>
					</th>
					<td style="width:25%">
			...
			<tr>
				...
				<th style="width:25%">
					<label><fmt:message	key="trafficInspectorJunior.experience" />:</label>
				</th>
				<td style="width:25%">
					<c:if test="${ not empty applicantPostAssoc.oneYearExpAsTransportAssociation}">
					<c:choose>
						<c:when
				...
				<c:if test="${ applicantPostAssoc.oneYearExpAsTransportAssociation eq true }">
						<th style="width:25%">
						<label><fmt:message
								key="trafficInspector.experienced.companyName" />:</label>
						</th>
						<td style="width:25%">
			...
			<tr>
				<th style="width:25%">
					<label><fmt:message key="trafficInspectorJunior.ownHeavyDrivingLicenseNo" />:</label>
				</th>
				<td style="width:25%">
				...
				<c:if
					...
					<th style="width: 25%"><c:if
							...
							<label><fmt:message
									key="trafficInspectorJunior.ownHeavyDrivingLicenseNo.number" />:</label>
						</c:if></th>
				...
				<tr>
					<c:if
						...
						<th style="width: 25%"><label> <fmt:message
									key="trafficInspectorJunior.ownHeavyDrivingLicenseNo.issueDate" />:
						</label></th>
					...
					<c:if
						...
						<th style="width: 25%"><label><fmt:message
									key="trafficInspectorJunior.ownHeavyDrivingLicenseNo.endDate" />:</label>
						</th>
						<td style="width: 25%"><c:if
								test="${ applicantPostAssoc.ownHeavyDrivingLicenseNumber eq true }">
								<label><tags:localDate pattern="dd/MM/yyyy"
