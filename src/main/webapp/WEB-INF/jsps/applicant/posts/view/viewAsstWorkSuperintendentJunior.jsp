	...
	<div class="row">
		<div class="col-xs-8">
			<strong><fmt:message key="paymentResponse.postAppliedFor" />
				:</strong>
			<fmt:message key="post.assistant.workshop.superintendent.junior" />
		</div>
		<div class="col-xs-4">
...
<form class="form-horizontal">
	<table style="width: 100%">
		<tr>
			<th style="width: 25%"><label><fmt:message
						key="asst.work.superintendent.engineer" />:</label></th>
			...
			<c:if
				test="${ applicantPostAssoc.automobileOrMechanicalEngDegree eq true }">
				<th style="width: 25%"><label class="control-label"
					for="degree"><fmt:message
		...
		<c:if
			...
			<tr>
				<th style="width: 25%"><label class="control-label"
					for="university"><fmt:message key="juniorEngineerCivil.universityName" />:</label></th>
				<td style="width: 25%"><c:if
						test="${ applicantPostAssoc.automobileOrMechanicalEngDegree eq true }">
	...
	<%-- 	<tr>
		...
		<tr>
			<th style="width: 25%"><label><fmt:message
						key="asst.work.superintendent.year.experience" />:</label></th>
			<td style="width: 25%"><c:if
			...
			<c:if
				test="${ applicantPostAssoc.oneYearExpInAutomobileRepairing eq true }">
				<th style="width: 25%"><label><fmt:message
							key="assistantTrafficSuperintendent.experienced.companyName" />:</label></th>
		...
		<tr>
			<th style="width: 25%"><label><fmt:message
						key="driver.ownHeavyDrivingLicenseNo" />:</label></th>
			<td style="width: 25%"><c:if
			 ...
			 <c:if
				...
				<c:if test="${(applicantPostAssoc.ownHeavyDrivingLicenseNumber eq true and  not empty applicantPostAssoc.heavyDrivingLicenseNumber) or  applicantPostAssoc.ownHeavyDrivingLicenseNumber eq false}">
				<label><fmt:message
							key="driver.ownHeavyDrivingLicenseNo.number" />:</label>
							</c:if></th>
				<td style="width: 25%">
						<c:choose>
							<c:when
		...
		<c:if
			...
			<tr>
				...
				<c:if
					...
					<th style="width: 25%"><label><fmt:message
								key="trafficInspectorJunior.ownHeavyDrivingLicenseNo.endDate" />:</label></th>
					<td style="width: 25%"><c:if
							test="${ applicantPostAssoc.ownHeavyDrivingLicenseNumber eq true }">
							<label><tags:localDate pattern="dd/MM/yyyy"
