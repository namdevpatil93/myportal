	...
	<div class="row">
		<div class="col-xs-8">
			<strong><fmt:message key="paymentResponse.postAppliedFor" />
				:</strong>
			<fmt:message key="post.driver.junior" />
		</div>
		<div class="col-xs-4">
...
<form class="form-horizontal">
	<table style="width: 100%">
		...
		<tr>
			...
			<td style="width: 25%"><c:if
			 ...
			 <c:if
				test="${ not empty applicantPostAssoc.ownConductorBatchNumber }">
				<th style="width: 25%">
				<c:if test="${(applicantPostAssoc.ownConductorBatchNumber eq true and  not empty applicantPostAssoc.conductorBatchNumber) or  applicantPostAssoc.ownConductorBatchNumber eq false}">
				<label><fmt:message
							key="driver.ownConductorbatchNo.number" />:</label></c:if></th>
				<td style="width: 25%">
						<c:choose>
							<c:when
								test="${ applicantPostAssoc.ownConductorBatchNumber eq true }">
		...
		<tr>
			<th style="width: 25%"><label><fmt:message
						key="driver.experienced" />:</label></th>
			<td style="width: 25%"><c:if
